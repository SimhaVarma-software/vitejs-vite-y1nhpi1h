...
const Form = () => {
  ...
  const handleChange = (event) => {
    setFormData({
      ...formData,
      [event.target.name]: event.target.value.trim(), // Prevents extra spaces
    });
  };
  ...
  return (
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="url" id="posterImageUrl" name="posterImageUrl" value={formData.posterImageUrl} onChange={handleChange} required />

        <div className="form-buttons">
          <button type="submit">Submit</button>
          <button type="button" onClick={() => navigate("/")}>Cancel</button> {/* Fixed navigation */}
        </div>
      </form>
    </div>
  );
